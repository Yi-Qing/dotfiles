...
set textwidth=120			"一行最多输入70字符，否则自动换行(会添加换行符，慎用), 可用gq强制换行
...
set rulerformat=%40(%2*%<%=%t%m%r\ %l,%c\ %p%%%) 
" 状态栏设置 end

" 文件编码 start
" set fileencodings=utf-8,gb2312,gbk,ucs-bom,cp936,gb18030,big5,euc-jp,euc-kr,latin1,utf-16,utf-32
...
"自动按照文件类型执行脚本函数
autocmd BufNewFile *.sh,*.c,*.h,*.hpp,*.cpp exec ":call SetTitle()" 
...
elseif (expand("%:e") == 'h') || (expand("%:e") == 'hpp')
	...
	call append(line(".")+1, "")
	call append(line(".")+2, "")
	call append(line(".")+3, "#endif // __".toupper(expand("%:t:r"))."_H__")
elseif &filetype == 'c'
...
autocmd TermOpen term://* set nornu
autocmd TermOpen term://* set nonu
